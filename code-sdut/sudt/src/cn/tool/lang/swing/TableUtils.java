...
public class TableUtils {
	 ...
	 * @param isMoveUp
	 */
	public static void moveRows(JTable table, boolean isMoveUp) {
	 ...
	 * @param values
	 */
	public static void appendRow(JTable table, Object ...values) {
	 ...
	 * @param values
	 */
	public static void insertRow(JTable table, int insertRow, Object ...values) {
	 ...
	 * @param values
	 */
	public static void updateRow(JTable table, int row, Object ...values) {
	 ...
	 * @param infoKey
	 */
	public static boolean deleteRow(JTable table, String infoKey) {
		int[] rows = table.getSelectedRows();
		if (null != rows && rows.length > 0) {
			...
			if (rows.length > 1) {
				...
				for (int i = 0; i < rows.length; i++) {
					DefaultTableModel model = (DefaultTableModel) table.getModel();
					selItem += model.getValueAt(rows[i], 0) + "\r\n";
				}
				selItem = selItem.substring(0, selItem.length() - 2);
			} else {
				DefaultTableModel model = (DefaultTableModel) table.getModel();
				selItem =  (String) model.getValueAt(rows[0], 0);
			}
			
			int option = JOptionPane.showConfirmDialog(null, String.format(
					ToolConfig.i18.getProperty(infoKey), selItem), 
	 ...
	 * @return
	 */
	private static int getMaxWidthForCloumn(JTable table, int col) {
		int maxw = 0;
	 ...
	 * @param col
	 */
	 public static void setCloumnWidth(JTable table, int col) {
